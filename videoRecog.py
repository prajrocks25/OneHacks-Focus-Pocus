...
while True:
    ...
    for(minX, minY, width, height) in faces:
        ...
        if((distractionStarting==True) and (time.time()-distractionStart>5)):
            ...
            cv2.putText(frame, "Sorry you were distracted", (60,60),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,0),3,cv2.LINE_4,False)
            cv2.waitKey(10)
            distracted = True
            times = int(end-start)
            ...
            cv2.putText(frame, "Time focused: " + str(Minutes)+":"+str(Seconds), (540,30),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,0),2,cv2.LINE_4,False)
            exit = True
            break; 

        if ((distractionStarting==True) and (time.time()-start>10) and (minX<bottomX-50 or minX>bottomX+50)):
            cv2.putText(frame, "Distracted: session will end in " + str(5-int(time.time()-distractionStart)), (460,30),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,0),2,cv2.LINE_4,False)
        else:
            distractionStarting=False
